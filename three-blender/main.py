...
for camera in bpy.data.cameras:
    ...
    cam_code += f"const {safe_name(camera.name)} = new THREE.PerspectiveCamera({bpy.context.object.data.lens}, window.innerWidth / window.innerHeight, 0.1, 1000);\n"
...
def loader(filepath, object):
    ...
    load_code += f"\t\t{safe_name(object.name)}.position.set({location.x}, {location.y}, {location.z});\n"
    load_code += f"\t\t{safe_name(object.name)}.rotation.set({rotation.x}, {rotation.y}, {rotation.z});\n"
